...
def training_metrics_closure(logger_metrics, epoch, global_step, loss, learning_rate, tp, grad_norm, param_norm):
    loss_val = loss.detach().to('cpu').item()
    grad_norm_val = grad_norm.detach().to('cpu').item()
    param_norm_val = param_norm.detach().to('cpu').item()
