...
class LlamaMLP(nn.Module):
    def __init__(self, config):
        ...
        # if not os.environ.get("NEURON_EXTRACT_GRAPHS_ONLY", None):
        #     global mlp_cnt
        #     xm.master_print(f"mlp_cnt={mlp_cnt}")
        #     mlp_state_dict = torch.load(f"{trn_state_path}/layer_{mlp_cnt}_mlp.pt")
        #     mlp_cnt += 1
        #     self.gate_proj.weight.data = get_sharded_data(mlp_state_dict["g_weight"], 0)
        #     self.up_proj.weight.data = get_sharded_data(mlp_state_dict["u_weight"], 0)
        #     self.down_proj.weight.data = get_sharded_data(mlp_state_dict["d_weight"], 1)
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        # if not os.environ.get("NEURON_EXTRACT_GRAPHS_ONLY", None):
        #     global attn_cnt
        #     xm.master_print(f"attn_cnt={attn_cnt}")
        #     attn_state_dict = torch.load(f"{trn_state_path}/layer_{attn_cnt}_attn.pt")
        #     attn_cnt += 1
        #     self.o_proj.weight.data = get_sharded_data(attn_state_dict["o_weight"], 1)
        #     self.rotary_emb.inv_freq.data = attn_state_dict["f_weight"]
        #     q_weight = get_sharded_data(attn_state_dict["q_weight"], 0)
        #     k_weight = get_sharded_data(attn_state_dict["k_weight"], 0)
        #     v_weight = get_sharded_data(attn_state_dict["v_weight"], 0)
        #     if self.num_heads == self.num_key_value_heads:
        #         self.qkv_proj.weight.data = torch.cat([q_weight, k_weight, v_weight], dim=0)
        #     else:
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        # if not os.environ.get("NEURON_EXTRACT_GRAPHS_ONLY", None):
        #     embed_int_state_dict = torch.load(f"{trn_state_path}/embed_in.pt")
        #     self.embed_tokens.weight.data = get_sharded_data(embed_int_state_dict["weight"], 0)
    ...
    def forward(
        ...
        # Haozheng: sequence parallel:
        if self.config.sequence_parallel_enabled:
            hidden_states = mappings.gather_from_sequence_parallel_region(hidden_states)
            hidden_states = hidden_states.transpose(0, 1).contiguous()
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # if not os.environ.get("NEURON_EXTRACT_GRAPHS_ONLY", None):
        #     lm_head_state_dict = torch.load(f"{trn_state_path}/lm_head.pt")
        #     self.lm_head.weight.data = get_sharded_data(lm_head_state_dict["weight"], 0)
