...
time neuron_parallel_compile python3 bert-jp-precompile.py

echo "### single worker 1st training"
sudo rmmod neuron; sudo modprobe neuron
time python3 bert-jp-single.py
...
sudo rmmod neuron; sudo modprobe neuron
time neuron_parallel_compile torchrun --nproc_per_node=2 bert-jp-dual.py

echo "### dual worker 1st dual training"
sudo rmmod neuron; sudo modprobe neuron
time torchrun --nproc_per_node=2 bert-jp-dual.py
